package tracks.singlePlayer.MovementsDetection;
...
public class Agent extends AbstractPlayer {
    ...
    public Agent(StateObservation so, ElapsedCpuTimer elapsedTimer)
    {
        ...
        HashMap<Types.ACTIONS, List<Types.ACTIONS>> hashmap = new HashMap();
        
        for(int i = 0; i < actions.length; i++){
            hashmap.put(actions[i], new ArrayList());
            
            StateObservation so_copy = so.copy();
            System.out.println("Testing movement "+printAction(actions[i]));
            ...
            if(!initial_pos.equals(so_copy.getAvatarPosition())){
                ...
                for(int j = 0; j < actions.length; j++){
                    ...
                    if(new_pos.equals(initial_pos)){
                        ...
                        addActionToHashMap(hashmap, actions[i], actions[j]);
                        fetched[i] = true;
                        break;
                    }
                }
            ...
            List<Types.ACTIONS> nulling_actions = hashmap.get(actions[i]);
    ...
    public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        System.out.println("Number of events: "+stateObs.getEventsHistory().size());
        
        for(Event e: stateObs.getEventsHistory()){
            // print something
        }
        //... and return it.
        return actions[m_rnd.nextInt(actions.length)];
    }
